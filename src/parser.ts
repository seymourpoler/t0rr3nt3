...
export class Parser {
    ...
    private getInfoPiecesFrom(content: string): Uint8Array {
        ...
        let start = position + key.length;
        let end = position + key.length;
        while (content[end] && /[0-9]/.test(content[end])) {
            end++;
        }
        if (start === end || content[end] !== ':') {
            return new Uint8Array([]);
        }
        const lengthOfThePiecesValue = parseInt(content.slice(start, end), 10);
        if (isNaN(lengthOfThePiecesValue) || lengthOfThePiecesValue < 20 || lengthOfThePiecesValue % 20 !== 0) {
            return new Uint8Array([]);
        }
        const positionOfTheValueOfPieces = end + 1;
        if (positionOfTheValueOfPieces + lengthOfThePiecesValue > content.length) return new Uint8Array([]);

        const valueOfPieces = content.substr(positionOfTheValueOfPieces, lengthOfThePiecesValue);
        const result = new Uint8Array(lengthOfThePiecesValue);
        for (let i = 0; i < lengthOfThePiecesValue; i++) {
            result[i] = valueOfPieces.charCodeAt(i) & 0xFF;
        }
